...
let start = document.querySelector('.refrigerator_start_button');
let score = document.querySelector('.refrigerator_score_field');
let scores = 0;
...
function draw(evt){
    ...
    btn.forEach(button => {
        if (button.id[0] === row || button.id[1] === column){
            if (button.style.background === "lightslategray") {
